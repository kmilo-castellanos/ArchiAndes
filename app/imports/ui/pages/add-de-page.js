...
Template.Add_DE_Page.events({
  'submit #AddDEQSForm': function(event) {
    event.preventDefault();
    //console.log("addding click");
    qsElement=QScenarios.findOne(FlowRouter.getParam('_id'));
    var nameValue = event.target.decisionName.value;
    var dataValue = event.target.data.value;
    var xmlValue = event.target.xml.value;
    //console.log(nameValue+","+xmlValue);
    ArchDecisions.insert({name:nameValue,
      data:dataValue,
      qs_name:qsElement.name,
      xml:xmlValue
    });
    FlowRouter.go("/add-aq/" + qsElement._id);
  },
});
